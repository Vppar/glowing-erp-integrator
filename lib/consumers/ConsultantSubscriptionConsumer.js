...
ConsultantSubscriptionConsumer.verifyNewSubscriptions = function () {	
	FirebaseBackend.on(Config.firebase.eventType.VALUE, consultantSubscriptionRequestQueueRef, function(err, subscriptionSnapshot) {
		...
		async.series([
			...
			if(err) {
				callback('[ConsultantSubscriptionConsumer.verifyNewSubscriptions][Consumer flow executed with errors: '+ err+']', null);
			} else {
				callback(null, '[ConsultantSubscriptionConsumer.verifyNewSubscriptions][Consumer flow executed successfully: '+ results+']');	
			}  		
		}); 		
	});			
};
