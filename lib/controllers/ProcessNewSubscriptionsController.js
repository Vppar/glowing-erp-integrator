        ...
        async.series([
          ...
          if(err) {
            SubscriptionService.updateSubscriptionStatus(subscriptionToProcess.uuid, GlowingSubscriptionStatus.ERROR, err, function(errorCallback) {
              if(errorCallback) {
                console.log('[ProcessNewSubscriptionsController.updateSubscriptionStatus][Subscription: '+subscriptionToProcess.uuid+'][Status: '+GlowingSubscriptionStatus.ERROR+'][Errors: '+errorCallback+']');
              } else {
                console.log('[ProcessNewSubscriptionsController.updateSubscriptionStatus][Subscription: '+subscriptionToProcess.uuid+'][Status: '+GlowingSubscriptionStatus.ERROR+'][Result OK');
              }
            });          
          } else {
