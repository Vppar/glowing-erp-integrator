...
SubscriptionStorage.updateGlowingBilletStatus = function (subscription, callback) {
  var subscriptionRef = FirebaseBackend.refs.base.child(Config.SUBSCRIPTION_REF+'/'+subscription.uuid+'/billet');
  var child = Config.billet.child.STATUS;
  FirebaseBackend.update(subscriptionRef, subscription.billet.status, child, function(err) {
	if(err) {
		callback(err, null);
	}
	callback();
...
SubscriptionStorage.updateBillet = function (subscriptionUUID, billet, callback) {
  ...
  FirebaseBackend.update(subscriptionRef, billet, 'billet', function(err) {
	if(err) {
		callback(err, null);
	}
	callback();
