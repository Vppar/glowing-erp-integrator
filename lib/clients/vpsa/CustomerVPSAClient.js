	...
	if(!customerDocument) {		
		...
		var req = Https.get(options, function(res) {	
			...
			res.on('data', function (body) {
				...
				if(body != Config.result.UNAUTHORIZEDACCESS && (body === Config.customer.EMPTY_RESULT || JSON.parse(body)[0].id)){
					console.log('[CustomerVPSAClient.getByDocument][Subscription: '+subscriptionUUID+'][MessageRS: '+JSON.stringify(body)+']');		 
				    callback(null, body);
				}else{
					console.log('[CustomerVPSAClient.getByDocument][Subscription: '+subscriptionUUID+'][Error: Unauthorized access]');	
					callback('[CustomerVPSAClient.getByDocument][Subscription: '+subscriptionUUID+'][Error: Unauthorized access]');	
				}
			})
		});
