...
/*CustomerService.update = function (subscriptionUUID, updateCustomer, idUpdateCustomer, callback) {
	if(!updateCustomer){
		...
		callback('[CustomerService.update][Subscription: '+subscriptionUUID+'][Error: idUpdateCustomer parameter is empty or invalid]');
	}else{
		CustomerVPSAClient.getCustomerById(idUpdateCustomer, function(err, result){
			if(err){
				callback(err);
			} else {				
				CustomerVPSAClient.update(subscriptionUUID, JSON.stringify(updateCustomer), idUpdateCustomer, function(err, success){
					if(err){
						callback(err);
					}else{
						callback(null, '[CustomerService.update][Subscription: '+subscriptionUUID+'][Success: Customer updated with success, id ' + success + ']');
					}
				});
			}
		});
...
CustomerService.create = function (subscriptionUUID, newCustomer, callback) {
	if(!newCustomer){
		...
		CustomerVPSAClient.create(subscriptionUUID, JSON.stringify(newCustomer), callback);
	}
};
...
CustomerService.mergeCustomer = function(subscription, callback){
	...
	if(!consultant || !consultant.cpf){
		...
		CustomerService.getByDocument(subscription.uuid, consultant.cpf, function(err, customerId){
			if(err) {
				callback(err);
			} else {
				CustomerService.adaptCustomerToTerceiroVPSA(subscription.uuid, consultant, function(err, terceiroVPSA){
					if(err) {
						callback(err);
					} else {
						if(customerId) {
							CustomerVPSAClient.update(subscription.uuid, terceiroVPSA, customerId, function(err, result){
								if(err){
									callback(err);
								}else{								
...
CustomerService.adaptCustomerToTerceiroVPSA = function(subscriptionUUID, consultant, callback){
	...
	if(!address || !address.codigoIBGECidade){
		...
		callback(null, JSON.stringify(terceiroVPSA));
	}
}
