	...
	if(!consultant){
		...
		CustomerService.getByDocument(subscription.uuid, consultant.cpf, function(err, result){
			if(err){
				...
				CustomerService.adaptCustomerToTerceiroVPSA(subscription.uuid, terceiroVPSA, consultant, function(err, result){
					if(err){
						callback(err);						
					}else{
				...
				CustomerService.adaptCustomerToTerceiroVPSA(subscription.uuid, terceiroVPSA, consultant, function(error, success){
					if(error){
						callback(error);
					}else{
...
CustomerService.adaptCustomerToTerceiroVPSA = function(subscriptionUUID, terceiroVPSA, consultant, callback){
	...
	if(!address|| !address.codigoIBGECidade){
		callback('[CustomerService.adaptCustomerToTerceiroVPSA][Subscription: '+subscriptionUUID+'][Error: Invalid consultant properties]');
	} else {			
...
function getAddress(subscriptionUUID, consultant) {
	...
	if(!consultant || !consultant.cep || !consultant.address || !consultant.address.street || consultant.address.street.split(' ')<2) {
		console.log('[CustomerService.adaptCustomerToTerceiroVPSA.getAddress][Subscription: '+subscriptionUUID+'][Error: Invalid consultant properties]');
		return null;
	}
