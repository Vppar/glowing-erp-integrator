	...
	if(!consultant){
		...
		CustomerService.getByDocument(consultant.cpf, function(err, result){
			if(err){
				...
				console.log('Terceiro não existente no vpsa');
				CustomerService.adaptCustomerToTerceiroVPSA(terceiroVPSA, consultant, function(err, result){
					if(err){
						callback(err);						
					}else{
						CustomerVPSAClient.create(JSON.stringify(terceiroVPSA), function(err, result){
							if(err){
								...
								console.log('Terceiro criado com sucesso');
				...
				CustomerService.adaptCustomerToTerceiroVPSA(terceiroVPSA, consultant, function(error, success){
					if(error){
						callback(error);
					}else{
						CustomerVPSAClient.update(JSON.stringify(terceiroVPSA), JSON.parse(result)[0].id, function(err, result){
							if(err){
								...
								console.log('Terceiro atualizado com sucesso');
...
CustomerService.adaptCustomerToTerceiroVPSA = function(terceiroVPSA, customer, callback){
	var cep = customer.cep.replace('.','').replace('-','').replace(' ','');
	var codIbge = IbgeUtil.codIbge[cep];
	
	if(!codIbge){
		callback('[CustomerService.adaptCustomerToTerceiroVPSA][Error: CEP doens\'t not exists]');
	}else{
		var endereco = {
			tipo: 'RUA',
