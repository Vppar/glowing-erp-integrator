...
EmailService.sendBilletToCustomer = function(billetRequestSnapshot, callback) {
	if (!EmailService.isValidBillet(billetRequestSnapshot)) {
		...
		fs.readFile( 'views/billetEmail.html', function (err, html) {
			if (err) {
				...
				var email = {
					"content": html.toString().replace("{billetLink}",billetRequestSnapshot.billetLink),
					"subject": "Assinatura VPink",
					"to": billetRequestSnapshot.consultant.email,
					"toName": billetRequestSnapshot.consultant.name,
				}
...
EmailService.sendApprovedPayment = function(billetRequestSnapshot, callback){
	if (!EmailService.isValidBillet(billetRequestSnapshot)) {
		callback('Invalid param');
	}
	else {
