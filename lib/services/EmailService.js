...
EmailService.sendApprovedPaymentNotification = function(subscription, callback){
	if('PAYED' === subscription.billet.status) {
		if (!EmailService.isValidBillet(subscription)) {
			...
			var emailToInformApprovedPayment = 'views/emailToInformApprovedPayment.html';
			if(subscription.renewal) {
				emailToInformApprovedPayment = 'views/emailToInformApprovedPaymentRenewal.html';
			} 
			FS.readFile(emailToInformApprovedPayment, function (err, html) {
				if (err) {
					callback(err);
				}else {
					var email = {
