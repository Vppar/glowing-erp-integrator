...
function getBilletToCreate(accountReceivableId) {
	return { idContaReceber: accountReceivableId, idCarteiraCobranca: Config.billet.CARTEIRADECOBRANCA };
}
		...
		OrderService.get(subscription.uuid, subscription.orderId, function(err, order){
			if(err) {
				...
				if(!order) {
					...
					BilletVPSAClient.createBillet(subscription.uuid, getBilletToCreate(order.parcelas[0].id), function(err, billetObj){
						if(err){
							callback(err);
						}else{		
...
BilletService.getBilletJSON = function (subscription, callback) {
	if(!subscription || !subscription.uuid || !subscription.billet || !subscription.billet.id || !subscription.billet.link || typeof subscription.billet.id !== 'number'){
		...
		BilletVPSAClient.getBilletJSON(subscription.uuid, subscription.billet.id, function(err, billetJSON) {
			if(err) {
				console.log('[BilletService.getBilletJSON][Subscription: '+subscription.uuid+'][Error: '+err+']');				 
				callback(err, null);
			} else {
				if(!billetJSON) {
					console.log('[BilletService.getBilletJSON][Subscription: '+subscription.uuid+'][Error: BilletJSON returned null]');
					callback('Error: BilletJSON returned null', null);
				} else {
...
BilletService.notifyPayedBilletToGlowingRestApi = function (subscription, callback) {
	if( !subscription || !subscription.uuid || !subscription.billet || !subscription.billet.status || !subscription.snapshot || !subscription.snapshot.consultant) {
		...
		if('PAYED' === subscription.billet.status) {
			...
			if(planType === GlowingSubscriptionPlanType.RIMEL.name || planType === GlowingSubscriptionPlanType.GLOSS.name || planType === GlowingSubscriptionPlanType.BLUSH.name){
			...
			var actualSubscriptionExpirationDate = subscriptionConsultantRequest.consultant.subscriptionExpirationDate;
			...
			newSubscriptionExpirationDate = ((actualSubscriptionExpirationDate && actualSubscriptionExpirationDate > new Date().getTime()) ?
											  DateTimeUtil.addUpMonths(actualSubscriptionExpirationDate, addUpMonths) : DateTimeUtil.addUpMonths(null, addUpMonths));
