...
BilletService.createBillet = function (subscription, callback) {
	if(!subscription){
		...
		OrderService.get(subscription.uuid, subscription.pedidoVendaId, function(err, order){
			if(err) {
				callback(err);
			} else {
				if(!order) {
					...
					BilletVPSAClient.createBillet(subscription.uuid, JSON.stringify(billet), function(err, billetObj){
						if(err){
							callback(err);
						}else{													
							subscription.billet.id = billetObj.idRecurso;
							subscription.billet.link = getBilletUrlPDF(subscription.uuid, resultObj.idRecurso); 
