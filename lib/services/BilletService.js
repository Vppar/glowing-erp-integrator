...
BilletService.updateBilletStatusByVPSABillet = function (subscription, billet, callback) {
	...
	if( !subscription || !subscription.uuid || !subscription.billet || !billet || !billet.status ) {
		callback('[BilletService.updateBilletStatus][Subscription: '+subscription.uuid+'][Errors: Invalid parameters to this flow][Subscription: '+JSON.stringify(subscription)+'][Billet: '+JSON.stringify(billet)+']', null);	
	} else {
		...
		SubscriptionStorage.updateBilletStatus(subscription.uuid, subscription.billet.status, function(err) {
		  if(err) {
			 console.log('[BilletController.updateBilletStatus][Subscription: '+subscription.uuid+'][Errors: '+err+']');
	       	 callback(err, null);
		  } else {
			 console.log('[BilletController.updateBilletStatus][Subscription: '+subscription.uuid+'][Result: Succefully executed]');				 
