				...
				BilletVPSAClient.createBillet(JSON.stringify(billet), function(err, result){
					if(err){
						...
						if(resultObj && resultObj.codigoMensagem == 0 && resultObj.idRecurso){
							...
							console.log('Campos id, link e status inseridos no subscription com sucesso');
...
BilletService.getBilletJSON = function (subscription, callback) {
	if(!subscription || !subscription.uuid || !subscription.billet || !subscription.billet.id || !subscription.billet.link || typeof subscription.billet.id != 'number'){
		...
		BilletVPSAClient.getBilletJSON(subscription.billet.id, function(err, billet) {
			if(err) {
				callback('[BilletService.getBilletJSON][Error: SubscriptionUUID: ' + subscription.uuid + '. ' + err + ']');
			} else {
				if(billet == '[]') {
					callback('[BilletService.getBilletJSON][Error: SubscriptionUUID: ' + subscription.uuid + '. Billet not found.]');
				} else {
					console.log('Boleto retornado com sucesso');
					
					var boleto = JSON.parse(billet);
					//boleto.status = 'PAGO';//apenas descomentar para teste
					
					callback(null, subscription, boleto);
				}
			}
		});
	}
...
BilletService.updateGlowingBilletStatus = function (subscription, billet, callback) {
	if( !subscription || !subscription.uuid || !subscription.billet || !billet || !billet.status ) {
		callback('[BilletService.updateGlowingBilletStatus][Errors: Invalid parameters to this flow][Subscription: '+JSON.stringify(subscription)+'][Billet: '+JSON.stringify(billet)+']');	
	} else {
		...
		if(billet.status === VPSABilletStatus.PAGO) {
			...
			callback ('[BilletService.updateGlowingBilletStatus][Error: Unable to define Glowing billet status by VPSA billet status ' + VPSABilletStatus + ']');
			return;
		}
		...
		SubscriptionStorage.updateGlowingBilletStatus(subscription.uuid, subscription.billet.status, function(err) {
			  if(err) {
				 ...
				 console.log('Status do boleto atualizado com sucesso');
...
BilletService.notifyPayedBilletToGlowingRestApi = function (subscription, callback) {
	if( !subscription || !subscription.uuid || !subscription.billet || !subscription.billet.status ||
		...
		callback('[BilletService.notifyPayedBilletToGlowingRestApi][Errors: Invalid parameters to this flow][Subscription: '+JSON.stringify(subscription)+']');	
	} else {
		if('PAYED' == subscription.billet.status) {
			...
			if(planType == GlowingSubscriptionPlanType.ONEMONTH.name || planType == GlowingSubscriptionPlanType.GLOSS.name || planType == GlowingSubscriptionPlanType.BLUSH.name){
				...
				callback('[BilletService.notifyPayedBilletToGlowingRestApi][Error updating subscription][Error: Unable to define days to addup for plan type '+planType+']');
				return;
			}
			...
			SubscriptionStorage.notifyPayedBilletToGlowingRestApi(subscription, function (err) {
				if(err) {
					...
					console.log('Glowing Catalog notificado do pagamento do boleto');
					callback(null, subscription);
				}			
			});
		} else {
			console.log('[BilletService.notifyPayedBilletToGlowingRestApi][Status: '+subscription.billet.status+'][Result: Not necessary notify]');
			callback(null, subscription);
		}
	}	
};
...
BilletService.markAsNotifiedToGlowing = function (subscription, callback) {
	if( !subscription || !subscription.uuid || !subscription.billet || !subscription.billet.status) {
		callback('[BilletService.markAsNotifiedToGlowing][Errors: Invalid parameters to this flow][Subscription: '+JSON.stringify(subscription)+']', null);	
	} else {
		if('PAYED' == subscription.billet.status) {
			SubscriptionStorage.updateGlowingSubscriptionStatus(subscription.uuid, GlowingSubscriptionStatus.PAYED_NOTIFIED, function(err) {
			  if(err) {
				 console.log('[BilletService.markAsNotifiedToGlowing][Errors: '+err+']');
			   	 callback(err, null);
			  } else {
				 console.log('[BilletService.markAsNotifiedToGlowing][Result: Succefully executed]');
			     callback(null, subscription);
			  }
			});
		} else {
...
BilletService.updateGlowingSubscriptionStatus = function (subscription, callback) {
	if( !subscription || !subscription.uuid || !subscription.billet || !subscription.billet.status) {
		callback('[BilletService.updateGlowingSubscriptionStatus][Errors: Invalid parameters to this flow][Subscription: '+JSON.stringify(subscription)+']');	
	} else {
		if(subscription.billet.status === 'PAYED' || 
			...
			callback(null, '[BilletService.updateGlowingSubscriptionStatus][Not necessary update. Billet Status: ' + subscription.billet.status + ']');	
		}
	}
};